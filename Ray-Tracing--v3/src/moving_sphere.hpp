...
public:
  ...
  auto bounding_box(double _time0, double _time1, aabb &output_box) const -> bool override;
...
auto moving_sphere::hit(const ray &r, double t_min, double t_max, hit_record &rec) const -> bool {
...
auto moving_sphere::bounding_box(double _time0, double _time1, aabb &output_box) const -> bool {
  aabb box0 = aabb(center(_time0) - Vec3d(radius, radius, radius), center(_time0) + Vec3d(radius, radius, radius));
  aabb box1 = aabb(center(_time1) - Vec3d(radius, radius, radius), center(_time1) + Vec3d(radius, radius, radius));
