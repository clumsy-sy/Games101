...
struct Vector2 {
  ...
  auto operator+(const Vector2 &right) const -> Vector2 {
  ...
  auto operator*(const float &other) const -> Vector2 {
...
struct Vector3 {
  ...
  // Addition Operator Overload
  auto operator+(const Vector3 &right) const -> Vector3 {
...
auto MagnitudeV3(const Vector3 in) -> float {
  return (sqrtf(powf(in.X, 2) + powf(in.Y, 2) + powf(in.Z, 2)));
}

// Vector3 DotProduct
auto DotV3(const Vector3 a, const Vector3 b) -> float {
  return (a.X * b.X) + (a.Y * b.Y) + (a.Z * b.Z);
}

// Angle between 2 Vector3 Objects
auto AngleBetweenV3(const Vector3 a, const Vector3 b) -> float {
...
auto ProjV3(const Vector3 a, const Vector3 b) -> Vector3 {
...
auto operator*(const float &left, const Vector3 &right) -> Vector3 {
  return {right.X * left, right.Y * left, right.Z * left};
}

// A test to see if P1 is on the same side as P2 of a line segment ab
auto SameSide(Vector3 p1, Vector3 p2, Vector3 a, Vector3 b) -> bool {
...
auto GenTriNormal(Vector3 t1, Vector3 t2, Vector3 t3) -> Vector3 {
  Vector3 u = t2 - t1;
  Vector3 v = t3 - t1;
...
inline auto tail(const std::string &in) -> std::string {
...
inline auto firstToken(const std::string &in) -> std::string {
  if (!in.empty()) {
    size_t token_start = in.find_first_not_of(" \t");
